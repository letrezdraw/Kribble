...
export class Drawing implements DrawingInterface {
    ...
    const executeOperation = async () => {
      ...
      switch (actionType) {
        ...
        case CanvasAction.CLEAR:
          this._clear();
          break;
        case CanvasAction.BATCH_LINE:
          if (points && points.length >= 2 && color && size) {
            for (let i = 1; i < points.length; i++) {
              this._line(points[i - 1]!, points[i]!, color, size);
            }
          }
          break;
        case CanvasAction.BATCH_ERASE:
          if (points && points.length >= 2 && size) {
            for (let i = 1; i < points.length; i++) {
              this._erase(points[i - 1]!, points[i]!, size);
            }
          }
          break;
        default:
  ...
  public normalizeCoordinate: DrawingInterface['normalizeCoordinate'] = (
    ...
    const bx = Math.min(Math.max(coord.x, 0), w);
    const by = Math.min(Math.max(coord.y, 0), h);
  ...
  public denormalizeCoordinate: DrawingInterface['denormalizeCoordinate'] = (
    ...
    const h = this._maxHeight;
    if (w <= 0 || h <= 0) return { x: 0, y: 0 };
    const nx = Math.min(Math.max(coord.x, 0), 1);
    const ny = Math.min(Math.max(coord.y, 0), 1);
