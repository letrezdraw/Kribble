/**
 * Kribble V2 - Production-Level Socket Handlers
 * Authoritative, reconnect-safe, identity-based multiplayer
 * 
 * CANVAS COMMAND PROTOCOL - Unified drawing system
 * All canvas operations go through canvas:command events
 */

import { Server, Socket } from 'socket.io';
import { v4 as uuidv4 } from 'uuid';
import { logger } from '../utils/logger.js';
import { validateUsername, validateMessage } from '../utils/profanityFilter.js';
import { guessRateLimiter, chatRateLimiter, drawRateLimiter } from '../utils/rateLimiter.js';

// Core modules
import {
  createRoom,
  joinRoom,
  handleDisconnect,
  handleIntentionalLeave,
  getRoom,
  getActiveRooms,
  getPlayerBySocket,
  getRoomIdByUser,
  forceLeaveCurrentRoom,
  socketToUser,
} from '../core/RoomManager.js';

import {
  startGame,
  transitionToWordSelection,
  startDrawingPhase,
  processGuess,
  revealHint,
  endTurn,
  endRound,
  endGame,
  resetGame,
} from '../core/GameStateMachine.js';

import { Room, Player, CanvasCommand, Stroke, Point } from '../types/game.js';

function clamp(value: number, min: number, max: number) {
  return Math.max(min, Math.min(max, value));
}

// Helper to serialize player for client
function serializePlayer(player: Player) {
  return {
    userId: player.userId,
    username: player.username,
    avatarId: player.avatarId,
    score: player.score,
    isHost: player.isHost,
    isDrawer: player.isDrawer,
    connected: player.connected,
    connectionState: player.connectionState,
  };
}

// Helper to serialize room for client
function serializeRoom(room: Room) {
  return {
    id: room.id,
    name: room.name,
    phase: room.phase,
    hostId: room.hostId,
    players: Array.from(room.players.values()).map(serializePlayer),
    drawerOrder: room.drawerOrder,
    currentDrawerIndex: room.currentDrawerIndex,
    roundNumber: room.roundNumber,
    totalRounds: room.totalRounds,
    turnTimer: room.turnTimer,
    wordSelectionTimer: room.wordSelectionTimer,
    // Never send secret word or word choices in bulk sync (drawer gets game:word-selection / game:word-reveal)
    currentWord: null,
    wordOptions: [] as string[],
    wordHints: room.wordHints,
    hintsRemaining: room.hintsRemaining,
    maxPlayers: room.settings.maxPlayers,
    settings: room.settings,
  };
}

function emitRoomState(io: Server, room: Room) {
  io.to(room.id).emit('room:state', {
    room: serializeRoom(room),
  });
}

function clearWordSelectionTimer(room: Room) {
  if (room.wordSelectionInterval) {
    clearInterval(room.wordSelectionInterval);
    room.wordSelectionInterval = null;
  }
}

function clearTurnTimer(room: Room) {
  if (room.turnTimerInterval) {
    clearInterval(room.turnTimerInterval);
    room.turnTimerInterval = null;
  }
}

function startWordSelectionBroadcast(io: Server, room: Room) {
  clearWordSelectionTimer(room);
  clearTurnTimer(room);

  room.wordSelectionTimer = room.settings.wordSelectionTime;
  emitRoomState(io, room);

  io.to(room.id).emit('game:word-selection', {
    wordOptions: [],
    selectionTime: room.wordSelectionTimer,
  });

  const drawerId = room.drawerOrder[room.currentDrawerIndex];
  const drawer = drawerId ? room.players.get(drawerId) : undefined;
  if (drawer?.socketId) {
    io.to(drawer.socketId).emit('game:word-selection', {
      wordOptions: room.wordOptions,
      selectionTime: room.wordSelectionTimer,
    });
  }

  room.wordSelectionInterval = setInterval(() => {
    if (room.phase !== 'wordSelection') {
      clearWordSelectionTimer(room);
      return;
    }

    room.wordSelectionTimer -= 1;
    io.to(room.id).emit('game:selection-timer', {
      timeRemaining: room.wordSelectionTimer,
    });

    if (room.wordSelectionTimer <= 0) {
      clearWordSelectionTimer(room);
      const fallbackWord = room.wordOptions[0];
      if (fallbackWord && startDrawingPhase(room, fallbackWord)) {
        startDrawingBroadcast(io, room);
      }
    }
  }, 1000);
}

function startDrawingBroadcast(io: Server, room: Room) {
  clearWordSelectionTimer(room);
  clearTurnTimer(room);

  emitRoomState(io, room);
  io.to(room.id).emit('game:drawing-started', {
    room: serializeRoom(room),
    wordLength: room.currentWord?.length ?? 0,
  });

  const drawerId = room.drawerOrder[room.currentDrawerIndex];
  const drawer = drawerId ? room.players.get(drawerId) : undefined;
  if (drawer?.socketId && room.currentWord) {
    io.to(drawer.socketId).emit('game:word-reveal', {
      word: room.currentWord,
    });
  }

  room.turnTimerInterval = setInterval(() => {
    if (room.phase !== 'drawing') {
      clearTurnTimer(room);
      return;
    }

    room.turnTimer -= 1;
    io.to(room.id).emit('game:timer-update', {
      timeRemaining: room.turnTimer,
    });

    if (room.turnTimer <= 0) {
      clearTurnTimer(room);
      finishTurnBroadcast(io, room);
    }
  }, 1000);
}

function finishTurnBroadcast(io: Server, room: Room) {
  if (room.phase !== 'drawing') {
    return;
  }

  const word = room.currentWord || '';
  const awards = [...room.turnAwards];
  endTurn(room);
  emitRoomState(io, room);

  const scores = Array.from(room.players.values()).map((player) => ({
    userId: player.userId,
    score: player.score,
  }));

  io.to(room.id).emit('game:turn-end', {
    word,
    awards,
    scores,
  });

  const roundComplete = room.currentDrawerIndex >= room.drawerOrder.length;
  if (roundComplete) {
    io.to(room.id).emit('game:round-end', { word, scores });
    setTimeout(() => {
      if (room.phase === 'turnEnd') {
        finishRoundBroadcast(io, room, word);
      }
    }, 3000);
    return;
  }

  setTimeout(() => {
    if (room.phase !== 'turnEnd') {
      return;
    }
    transitionToWordSelection(room);
    startWordSelectionBroadcast(io, room);
  }, 3000);
}

function finishRoundBroadcast(io: Server, room: Room, word?: string) {
  if (room.phase !== 'turnEnd' && room.phase !== 'drawing') {
    return;
  }

  endRound(room);
  emitRoomState(io, room);

  const scores = Array.from(room.players.values()).map((player) => ({
    userId: player.userId,
    score: player.score,
  }));

  io.to(room.id).emit('game:round-end', {
    word: word || room.currentWord || '',
    scores,
  });

  const isFinalRound = room.roundNumber >= room.totalRounds;
  if (isFinalRound) {
    setTimeout(() => {
      endGame(room);
      emitRoomState(io, room);

      const rankings = Array.from(room.players.values())
        .map((player) => ({
          userId: player.userId,
          username: player.username,
          score: player.score,
          avatarId: player.avatarId,
          rank: 0,
        }))
        .sort((a, b) => b.score - a.score)
        .map((player, index) => ({ ...player, rank: index + 1 }));

      io.to(room.id).emit('game:end', {
        finalScores: rankings.map(({ userId, username, score, avatarId }) => ({
          userId,
          username,
          score,
          avatarId,
        })),
        rankings,
      });
    }, 5000);
    return;
  }

  setTimeout(() => {
    if (room.phase !== 'roundEnd') {
      return;
    }
    room.roundNumber += 1;
    room.currentDrawerIndex = 0;
    room.drawerOrder = [...room.drawerOrder].sort(() => Math.random() - 0.5);
    transitionToWordSelection(room);
    startWordSelectionBroadcast(io, room);
  }, 5000);
}

export function setupSocketHandlers(io: Server) {
  io.on('connection', (socket: Socket) => {
    logger.info('SOCKET', 'Client connected', { socketId: socket.id });

    // ==========================================
    // LOBBY
    // ==========================================

    socket.on('lobby:get-rooms', () => {
      try {
        const activeRooms = getActiveRooms().map(room => ({
          id: room.id,
          name: room.name,
          hostName: room.players.get(room.hostId)?.username,
          playerCount: room.players.size,
          maxPlayers: room.settings.maxPlayers,
          phase: room.phase,
          isPrivate: room.settings.isPrivate,
          gameMode: room.settings.gameMode,
        }));
        
        socket.emit('lobby:rooms', { rooms: activeRooms });
      } catch (error) {
        logger.error('SOCKET', 'Error fetching rooms', error as Error);
        socket.emit('lobby:error', { message: 'Failed to fetch rooms' });
      }
    });

    // ==========================================
    // ROOM MANAGEMENT
    // ==========================================

    socket.on('room:create', (data: {
      name: string;
      settings?: Partial<Room['settings']>;
      username?: string;
      userId?: string;
      avatarId?: string;
    }) => {
      try {
        // Validate username
        const username = data.username || 'Player';
        const validation = validateUsername(username);
        if (!validation.valid) {
          socket.emit('room:error', { message: validation.error });
          return;
        }

        const userId = data.userId || uuidv4();
        const avatarId = data.avatarId || '👤';

        const room = createRoom(
          socket.id,
          userId,
          username,
          avatarId,
          data.name || 'Game Room',
          data.settings
        );

        if (!room) {
          socket.emit('room:error', { message: 'Failed to create room' });
          return;
        }

        socket.join(room.id);

        // Send room data to creator
        socket.emit('room:created', {
          room: serializeRoom(room),
          userId,
          isHost: true,
        });

        // Notify lobby
        io.emit('lobby:rooms-updated', {
          rooms: getActiveRooms().map(r => ({
            id: r.id,
            name: r.name,
            hostName: r.players.get(r.hostId)?.username,
            playerCount: r.players.size,
            maxPlayers: r.settings.maxPlayers,
            phase: r.phase,
          })),
        });

        logger.info('SOCKET', 'Room created', { roomId: room.id, userId });

      } catch (error) {
        logger.error('SOCKET', 'Error creating room', error as Error);
        socket.emit('room:error', { message: 'Internal error' });
      }
    });

    socket.on('room:join', (data: {
      roomId: string;
      password?: string;
      username?: string;
      userId?: string;
      avatarId?: string;
      joinByCode?: boolean;
    }) => {
      try {
        const username = data.username || 'Player';
        const validation = validateUsername(username);
        if (!validation.valid) {
          socket.emit('room:error', { message: validation.error });
          return;
        }

        const userId = data.userId || uuidv4();
        const avatarId = data.avatarId || '👤';

        // Try to find room
        let room = getRoom(data.roomId);

        // If not found and joinByCode, try partial match
        if (!room && data.joinByCode) {
          const allRooms = getActiveRooms();
          const match = allRooms.find(r => r.id.startsWith(data.roomId.toUpperCase()));
          if (match) {
            room = match;
          }
        }

        if (!room) {
          socket.emit('room:error', { message: 'Room not found' });
          return;
        }

        // Check password for private rooms
        if (room.settings.isPrivate && room.settings.password) {
          if (data.password !== room.settings.password) {
            socket.emit('room:error', { message: 'Incorrect password' });
            return;
          }
        }

        const existingPlayer = room.players.get(userId);
        if (!existingPlayer && room.phase !== 'waiting') {
          socket.emit('room:error', { message: 'This room already started. Join before the host starts the game.' });
          return;
        }

        const result = joinRoom(socket.id, userId, username, avatarId, room.id);

        if (!result) {
          socket.emit('room:error', { message: 'This player is already active in the room. Use a different account/window or reconnect the original session.' });
          return;
        }

        const { room: joinedRoom, player, isReconnect } = result;

        socket.join(joinedRoom.id);

        // Send room data to joining player
        socket.emit('room:joined', {
          room: serializeRoom(joinedRoom),
          userId,
          isHost: player.isHost,
          isReconnect,
        });

        // If game in progress, send both legacy stroke history and command history.
        if (joinedRoom.phase === 'drawing' || joinedRoom.phase === 'wordSelection') {
          const commands = joinedRoom.commandHistory || [];
          const strokes = joinedRoom.canvasHistory || [];
          socket.emit('canvas:sync', { commands, strokes });

          if (player.isDrawer && joinedRoom.phase === 'drawing' && joinedRoom.currentWord) {
            socket.emit('game:word-reveal', { word: joinedRoom.currentWord });
          }
          if (player.isDrawer && joinedRoom.phase === 'wordSelection') {
            socket.emit('game:word-selection', {
              wordOptions: joinedRoom.wordOptions,
              selectionTime: joinedRoom.wordSelectionTimer,
            });
          }
          
          logger.info('CANVAS', 'Sent command history to late joiner', {
            roomId: joinedRoom.id,
            userId,
            commandCount: commands.length,
            strokeCount: strokes.length,
            phase: joinedRoom.phase,
          });
        }

        // Notify other players
        socket.to(joinedRoom.id).emit('room:player-joined', {
          player: serializePlayer(player),
          isReconnect,
          room: serializeRoom(joinedRoom),
        });
        emitRoomState(io, joinedRoom);

        // System chat message for join
        if (!isReconnect) {
          io.to(joinedRoom.id).emit('chat:message', {
            userId: 'system',
            username: 'System',
            message: `${player.username} joined the room`,
            isSystem: true,
            timestamp: Date.now(),
          });
        }

        // Notify lobby
        io.emit('lobby:rooms-updated', {
          rooms: getActiveRooms().map(r => ({
            id: r.id,
            name: r.name,
            hostName: r.players.get(r.hostId)?.username,
            playerCount: r.players.size,
            maxPlayers: r.settings.maxPlayers,
            phase: r.phase,
            isPrivate: r.settings.isPrivate,
            gameMode: r.settings.gameMode,
          })),
        });

        logger.info('SOCKET', 'Player joined room', {
          roomId: joinedRoom.id,
          userId,
          isReconnect,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error joining room', error as Error);
        socket.emit('room:error', { message: 'Internal error' });
      }
    });

    socket.on('room:leave', () => {
      try {
        const result = handleIntentionalLeave(socket.id);
        if (!result) return;

        const { roomId, userId, player, roomEmpty } = result;

        socket.leave(roomId);

        // Get updated room to send full state
        const updatedRoom = getRoom(roomId);
        
        // Notify other players with full room state
        if (updatedRoom && !roomEmpty) {
          io.to(roomId).emit('room:player-left', {
            userId,
            username: player.username,
            newHostId: updatedRoom.hostId,
            room: serializeRoom(updatedRoom),
          });
          emitRoomState(io, updatedRoom);

          // System chat message for leave
          io.to(roomId).emit('chat:message', {
            userId: 'system',
            username: 'System',
            message: `${player.username} left the room`,
            isSystem: true,
            timestamp: Date.now(),
          });
        } else {
          io.to(roomId).emit('room:player-left', {
            userId,
            username: player.username,
            newHostId: undefined,
          });
        }

        // Notify lobby
        io.emit('lobby:rooms-updated', {
          rooms: getActiveRooms().map(r => ({
            id: r.id,
            name: r.name,
            hostName: r.players.get(r.hostId)?.username,
            playerCount: r.players.size,
            maxPlayers: r.settings.maxPlayers,
            phase: r.phase,
          })),
        });

        logger.info('SOCKET', 'Player left room', { roomId, userId, roomEmpty, newHostId: updatedRoom?.hostId });

      } catch (error) {
        logger.error('SOCKET', 'Error leaving room', error as Error);
      }
    });

    socket.on('room:update-settings', (data: {
      roomId: string;
      settings: Partial<Room['settings']> & { rounds?: number };
    }) => {
      try {
        const room = getRoom(data.roomId);
        if (!room) {
          socket.emit('room:error', { message: 'Room not found' });
          return;
        }

        const player = room.players.get(getPlayerBySocket(socket.id)?.player.userId || '');
        if (!player || !player.isHost) {
          socket.emit('room:error', { message: 'Only host can update settings' });
          return;
        }

        if (room.phase !== 'waiting') {
          socket.emit('room:error', { message: 'Settings can only be changed before the game starts' });
          return;
        }

        const requestedRounds = typeof data.settings.rounds === 'number'
          ? data.settings.rounds
          : data.settings.totalRounds;

        room.settings.maxPlayers = typeof data.settings.maxPlayers === 'number'
          ? clamp(data.settings.maxPlayers, 2, 16)
          : room.settings.maxPlayers;
        room.settings.roundTime = typeof data.settings.roundTime === 'number'
          ? clamp(data.settings.roundTime, 30, 300)
          : room.settings.roundTime;
        room.settings.wordSelectionTime = typeof data.settings.wordSelectionTime === 'number'
          ? clamp(data.settings.wordSelectionTime, 5, 60)
          : room.settings.wordSelectionTime;
        room.settings.totalRounds = typeof requestedRounds === 'number'
          ? clamp(requestedRounds, 1, 10)
          : room.settings.totalRounds;
        room.settings.hints = typeof data.settings.hints === 'number'
          ? clamp(data.settings.hints, 0, 5)
          : room.settings.hints;
        room.settings.wordCount = typeof data.settings.wordCount === 'number'
          ? clamp(data.settings.wordCount, 2, 6)
          : room.settings.wordCount;
        room.settings.categories = Array.isArray(data.settings.categories) && data.settings.categories.length > 0
          ? data.settings.categories
          : room.settings.categories;
        room.settings.language = typeof data.settings.language === 'string' && data.settings.language.trim()
          ? data.settings.language
          : room.settings.language;
        room.settings.gameMode = typeof data.settings.gameMode === 'string' && data.settings.gameMode.trim()
          ? data.settings.gameMode
          : room.settings.gameMode;
        room.settings.isPrivate = typeof data.settings.isPrivate === 'boolean'
          ? data.settings.isPrivate
          : room.settings.isPrivate;

        if (typeof data.settings.password === 'string') {
          room.settings.password = data.settings.password || undefined;
        }

        room.totalRounds = room.settings.totalRounds;
        room.turnTimer = room.settings.roundTime;
        room.wordSelectionTimer = room.settings.wordSelectionTime;
        room.hintsRemaining = room.settings.hints;
        room.updatedAt = Date.now();

        // Notify all players
        io.to(room.id).emit('room:settings-updated', {
          settings: room.settings,
        });
        emitRoomState(io, room);

        io.emit('lobby:rooms-updated', {
          rooms: getActiveRooms().map(r => ({
            id: r.id,
            name: r.name,
            hostName: r.players.get(r.hostId)?.username,
            playerCount: r.players.size,
            maxPlayers: r.settings.maxPlayers,
            phase: r.phase,
            isPrivate: r.settings.isPrivate,
            gameMode: r.settings.gameMode,
          })),
        });

        logger.info('SOCKET', 'Room settings updated', {
          roomId: room.id,
          settings: room.settings,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error updating settings', error as Error);
      }
    });

    // ==========================================
    // GAME MANAGEMENT
    // ==========================================

    socket.on('game:start', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) {
          socket.emit('game:error', { message: 'Not in a room' });
          return;
        }

        const { room, player } = result;

        if (!player.isHost) {
          socket.emit('game:error', { message: 'Only host can start game' });
          return;
        }

        const connectedPlayers = Array.from(room.players.values()).filter((candidate) => candidate.connected).length;
        const allowSoloStart = room.settings.gameMode === 'solo' || room.settings.maxPlayers === 1;
        if (!allowSoloStart && connectedPlayers < 2) {
          socket.emit('game:error', { message: 'At least 2 connected players are required to start the game' });
          return;
        }

        const success = startGame(room);
        if (!success) {
          socket.emit('game:error', {
            message: room.settings.gameMode === 'solo' || room.settings.maxPlayers === 1
              ? 'Cannot start game'
              : 'At least 2 connected players are required to start the game',
          });
          return;
        }

        // Clear command history when game starts (fresh canvas)
        room.commandHistory = [];

        // Notify all players
        io.to(room.id).emit('game:started', {
          room: serializeRoom(room),
        });

        if (room.phase === 'wordSelection') {
          startWordSelectionBroadcast(io, room);
        } else if (room.phase === 'drawing' && room.settings.gameMode === 'solo') {
          startDrawingBroadcast(io, room);
        }

        logger.info('SOCKET', 'Game started', { roomId: room.id });

      } catch (error) {
        logger.error('SOCKET', 'Error starting game', error as Error);
      }
    });

    socket.on('game:select-word', (data: { word: string }) => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        if (!player.isDrawer) {
          socket.emit('game:error', { message: 'Only drawer can select word' });
          return;
        }

        const success = startDrawingPhase(room, data.word);
        if (!success) return;

        startDrawingBroadcast(io, room);

        logger.info('SOCKET', 'Word selected, drawing started', {
          roomId: room.id,
          word: data.word,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error selecting word', error as Error);
      }
    });

    socket.on('game:request-hint', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        // Only non-drawers can request hints
        if (player.isDrawer) return;

        const hints = revealHint(room);
        if (!hints) return;

        // Notify all players
        io.to(room.id).emit('game:hint-revealed', {
          hints,
          hintsRemaining: room.hintsRemaining,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error requesting hint', error as Error);
      }
    });

    socket.on('game:play-again', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        if (!player.isHost) {
          socket.emit('game:error', { message: 'Only host can reset game' });
          return;
        }

        const success = resetGame(room);
        if (!success) return;

        // Clear command history for new game
        room.commandHistory = [];

        // Notify all players
        io.to(room.id).emit('game:reset', {
          room: serializeRoom(room),
        });

        logger.info('SOCKET', 'Game reset', { roomId: room.id });

      } catch (error) {
        logger.error('SOCKET', 'Error resetting game', error as Error);
      }
    });

    // ==========================================
    // GUESSING
    // ==========================================

    socket.on('guess:submit', (data: { guess: string }) => {
      try {
        // Rate limiting
        if (!guessRateLimiter.canProceed(socket.id)) {
          socket.emit('guess:error', { message: 'Too many guesses' });
          return;
        }

        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        // Validate guess
        const guessValidation = validateMessage(data.guess);
        if (!guessValidation.valid) {
          socket.emit('guess:error', { message: guessValidation.error });
          return;
        }

        const guessResult = processGuess(room, player.userId, data.guess);

        if (guessResult.correct) {
          // Notify guesser
          socket.emit('guess:correct', {
            points: guessResult.points,
            totalScore: player.score,
          });

          // Notify all players
          io.to(room.id).emit('game:player-guessed', {
            userId: player.userId,
            username: player.username,
            points: guessResult.points,
            scores: Array.from(room.players.values()).map(p => ({
              userId: p.userId,
              score: p.score,
            })),
          });

          // Check if all guessed
          const nonDrawers = Array.from(room.players.values()).filter(p => !p.isDrawer && p.connected);
          const allGuessed = nonDrawers.every(p => p.guessedThisTurn);

          if (allGuessed) {
            io.to(room.id).emit('game:all-guessed');
            setTimeout(() => {
              finishTurnBroadcast(io, room);
            }, 1100);
          }

        } else {
          // Wrong guess - broadcast to chat
          io.to(room.id).emit('chat:message', {
            userId: player.userId,
            username: player.username,
            message: data.guess,
            isGuess: true,
            timestamp: Date.now(),
          });
        }

      } catch (error) {
        logger.error('SOCKET', 'Error submitting guess', error as Error);
      }
    });

    // ==========================================
    // LEGACY STROKE CANVAS (Original 2.0-style sync)
    // ==========================================

    socket.on('draw:stroke', (data: { stroke: Stroke }) => {
      try {
        if (!drawRateLimiter.canProceed(socket.id)) {
          return;
        }

        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        if (!player.isDrawer || room.phase !== 'drawing') {
          return;
        }

        room.canvasHistory.push(data.stroke);
        room.canvasRedoHistory = [];

        socket.to(room.id).emit('draw:stroke', {
          playerId: player.userId,
          stroke: data.stroke,
        });
      } catch (error) {
        logger.error('SOCKET', 'Error handling draw:stroke', error as Error);
      }
    });

    socket.on('draw:stroke:chunk', (data: { strokeId: string; points: Point[]; tool: string; color: string; size: number }) => {
      try {
        if (!drawRateLimiter.canProceed(socket.id)) {
          return;
        }

        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;
        if (!player.isDrawer || room.phase !== 'drawing') {
          return;
        }

        socket.to(room.id).emit('draw:stroke:chunk', {
          playerId: player.userId,
          strokeId: data.strokeId,
          points: data.points,
          tool: data.tool,
          color: data.color,
          size: data.size,
        });
      } catch (error) {
        logger.error('SOCKET', 'Error handling draw:stroke:chunk', error as Error);
      }
    });

    socket.on('draw:clear', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;
        room.canvasRedoHistory = [...room.canvasHistory];
        room.canvasHistory = [];

        io.to(room.id).emit('draw:clear', { playerId: player.userId });
        emitRoomState(io, room);
      } catch (error) {
        logger.error('SOCKET', 'Error handling draw:clear', error as Error);
      }
    });

    socket.on('draw:undo', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;
        const removed = room.canvasHistory.pop();
        if (removed) {
          room.canvasRedoHistory.push(removed);
        }

        socket.to(room.id).emit('draw:undo', { playerId: player.userId });
        emitRoomState(io, room);
      } catch (error) {
        logger.error('SOCKET', 'Error handling draw:undo', error as Error);
      }
    });

    socket.on('draw:redo', () => {
      try {
        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;
        const restored = room.canvasRedoHistory.pop();
        if (restored) {
          room.canvasHistory.push(restored);
        }

        socket.to(room.id).emit('draw:redo', { playerId: player.userId });
        emitRoomState(io, room);
      } catch (error) {
        logger.error('SOCKET', 'Error handling draw:redo', error as Error);
      }
    });

    // ==========================================
    // CANVAS COMMAND PROTOCOL (Unified System)
    // ==========================================

    // Handle canvas commands from drawer
    socket.on('canvas:command', (data: { command: CanvasCommand }) => {
      try {
        // Rate limiting for drawing commands
        if ((data.command.type === 'START_STROKE' || data.command.type === 'ADD_POINTS') &&
            !drawRateLimiter.canProceed(socket.id)) {
          return;
        }

        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        // Only drawer can send canvas commands during drawing phase
        if (!player.isDrawer) {
          socket.emit('canvas:error', { message: 'Only drawer can draw' });
          return;
        }

        if (room.phase !== 'drawing') {
          socket.emit('canvas:error', { message: 'Not in drawing phase' });
          return;
        }

        // Store command in room history for replay/sync
        if (!room.commandHistory) {
          room.commandHistory = [];
        }
        room.commandHistory.push(data.command);

        // Broadcast to other players (not sender)
        socket.to(room.id).emit('canvas:command', {
          playerId: player.userId,
          command: data.command,
        });

        logger.debug('CANVAS', 'Command processed', {
          roomId: room.id,
          type: data.command.type,
          userId: player.userId,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error handling canvas command', error as Error);
        socket.emit('canvas:error', { message: 'Failed to process command' });
      }
    });

    // Handle canvas sync request (for late joiners/reconnects)
    socket.on('canvas:request-sync', (data: { roomId: string }) => {
      try {
        const room = getRoom(data.roomId);
        if (!room) {
          socket.emit('canvas:error', { message: 'Room not found' });
          return;
        }

        // Send both sync formats so either canvas client can restore state.
        const commands = room.commandHistory || [];
        const strokes = room.canvasHistory || [];
        socket.emit('canvas:sync', { commands, strokes });

        logger.info('CANVAS', 'Sync requested', {
          roomId: data.roomId,
          userId: socketToUser.get(socket.id),
          commandCount: commands.length,
          strokeCount: strokes.length,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error handling canvas sync request', error as Error);
      }
    });

    // ==========================================
    // CHAT
    // ==========================================

    socket.on('chat:message', (data: { message: string }) => {
      try {
        // Rate limiting
        if (!chatRateLimiter.canProceed(socket.id)) {
          socket.emit('chat:error', { message: 'Too many messages' });
          return;
        }

        const result = getPlayerBySocket(socket.id);
        if (!result) return;

        const { room, player } = result;

        // Validate message
        const validation = validateMessage(data.message);
        if (!validation.valid) {
          socket.emit('chat:error', { message: validation.error });
          return;
        }

        const censoredMessage = validation.censored || data.message;

        // Broadcast to all (including sender for consistency)
        io.to(room.id).emit('chat:message', {
          userId: player.userId,
          username: player.username,
          message: censoredMessage,
          isSystem: false,
          timestamp: Date.now(),
        });

      } catch (error) {
        logger.error('SOCKET', 'Error handling chat', error as Error);
      }
    });

    // ==========================================
    // DISCONNECT
    // ==========================================

    socket.on('disconnect', () => {
      try {
        const result = handleDisconnect(socket.id);
        if (!result) {
          logger.info('SOCKET', 'Disconnected socket not in room', { socketId: socket.id });
          return;
        }

        const { roomId, userId, player } = result;

        // Notify other players
        io.to(roomId).emit('room:player-disconnected', {
          userId,
          username: player.username,
          connectionState: player.connectionState,
        });
        const currentRoom = getRoom(roomId);
        if (currentRoom) {
          emitRoomState(io, currentRoom);
        }

        logger.info('SOCKET', 'Player disconnected', {
          roomId,
          userId,
          username: player.username,
        });

      } catch (error) {
        logger.error('SOCKET', 'Error handling disconnect', error as Error);
      }
    });
  });
}
