...
import { addMatchHistory, updatePlayerStats, getPlayerStats } from '../db/index.js';
import { rooms, getRoom, deleteRoom, createRoom } from '../data/rooms.js';
...
function generateHints(word, count) {
    const hints = new Array(word.length).fill('_');
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('hint:request', () => {
            ...
            if (!roomId)
                return;
            const room = getRoom(roomId);
            if (!room || room.gameState.hintsRemaining <= 0)
                return;
...
const roomsPendingDeletion = new Map();
// Track players pending removal (grace period for reconnection)
const playersPendingRemoval = new Map();
// Track drawer disconnection state
const drawerDisconnectionState = new Map();
...
function handlePlayerLeave(socket, io, isIntentional = false) {
...
function resumeRoundTimer(room, io) {
...
const wordSelectionCountdowns = new Map();
function startWordSelection(room, io) {
...
function startDrawingPhase(room, io) {
