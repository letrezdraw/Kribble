    ...
    prepare(sql) {
        ...
        return {
            get: (...params) => {
                ...
                if (lowerSql.includes('from users')) {
                    ...
                    if (lowerSql.includes('where email =') || lowerSql.includes('where username =')) {
                        for (const user of Object.values(this.data.users)) {
                            const u = user;
                            if (lowerSql.includes('email') && u.email === params[0])
                                return u;
                            if (lowerSql.includes('username') && u.username === params[0])
                                return u;
                        }
                        return null;
                    }
                ...
                if (lowerSql.includes('from word_categories')) {
                    ...
                    if (lowerSql.includes('select id, name')) {
                        return Object.values(this.data.word_categories).map((c) => ({ id: c.id, name: c.name }));
                    }
                    if (lowerSql.includes('select words')) {
                        return Object.values(this.data.word_categories).map((c) => ({ words: JSON.stringify(c.words) }));
                    }
                    if (lowerSql.includes('count(*)')) {
            ...
            all: (...params) => {
                ...
                if (lowerSql.includes('from users')) {
                    // Leaderboard query
                    const users = Object.values(this.data.users).map((u) => ({
                        ...u,
                    ...
                    return users.sort((a, b) => b.xp - a.xp).slice(0, params[0] || 100);
                }
                return [];
            },
            run: (...params) => {
                ...
                if (lowerSql.includes('update users')) {
                    ...
                    if (this.data.users[userId]) {
                        if (lowerSql.includes('username'))
                            this.data.users[userId].username = params[0];
                        if (lowerSql.includes('avatar_id'))
                            this.data.users[userId].avatar_id = params[0];
                        this.save();
                        return { changes: 1 };
                    }
                    return { changes: 0 };
                ...
                if (lowerSql.includes('insert into match_history')) {
                    ...
                    const userMatches = this.data.match_history.filter((m) => m.user_id === userId);
                    if (userMatches.length > 50) {
                        const toRemove = userMatches.slice(50).map((m) => m.id);
                        this.data.match_history = this.data.match_history.filter((m) => !toRemove.includes(m.id));
                    }
                    this.save();
                    return { lastInsertRowid: id, changes: 1 };
                }
...
export async function getPlayerStats(userId) {
    const stats = await queryOne('SELECT * FROM player_stats WHERE user_id = ?', [userId]);
    if (!stats) {
        return {
            userId,
            gamesPlayed: 0,
...
export async function incrementPlayerStat(userId, field, value = 1) {
...
export async function addMatchHistory(userId, match) {
  ...
  `, [
    ...
    const matches = await query('SELECT id FROM match_history WHERE user_id = ? ORDER BY played_at DESC', [userId]);
    if (Array.isArray(matches) && matches.length > 50) {
        const idsToDelete = matches.slice(50).map((m) => m.id);
        if (isPostgres) {
            await pgPool.query('DELETE FROM match_history WHERE id = ANY($1)', [idsToDelete]);
        }
        else {
...
export async function getMatchHistory(userId, limit = 20) {
    const matches = await query('SELECT * FROM match_history WHERE user_id = ? ORDER BY played_at DESC LIMIT ?', [userId, limit]);
...
export async function getLeaderboard(limit = 100) {
    ...
    LIMIT ?
  `, [limit]);
    const entries = (Array.isArray(users) ? users : []).map((user, index) => {
...
export async function getDailyChallenges(userId) {
    ...
    const existing = await query('SELECT * FROM daily_challenges WHERE user_id = ? AND challenge_date = ?', [userId, today]);
...
export async function updateChallengeProgress(userId, challengeId, progress) {
    const today = new Date().toISOString().split('T')[0];
    const challenge = await queryOne('SELECT * FROM daily_challenges WHERE user_id = ? AND challenge_date = ? AND challenge_id = ?', [userId, today, challengeId]);
...
export async function getWordsByCategory(categoryId) {
    if (categoryId && categoryId !== 'all') {
        const cat = await queryOne('SELECT words FROM word_categories WHERE id = ?', [categoryId]);
