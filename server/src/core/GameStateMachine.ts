 ...
 */
function calculateTurnScores(room: Room): { userId: string; points: number }[] {
  ...
  const correctGuessers = Array.from(room.players.values())
    ...
    .sort((a, b) => (a.joinAt - b.joinAt)); // Sort by guess order
  
  const timeRemaining = room.turnTimer;
  
  correctGuessers.forEach((player, index) => {
    const basePoints = Math.max(100 - (index * 10), 10); // First: 100, Second: 90, etc.
    const timeBonus = Math.floor(timeRemaining / 10);
    const totalPoints = basePoints + timeBonus;
    
    player.score += totalPoints;
    results.push({ userId: player.userId, points: totalPoints });
  });
...
export function startGame(room: Room): boolean {
  ...
  if (connectedPlayers === 1) {
    ...
    logger.info('GAME', 'Solo mode started', { roomId: room.id, userId: player.userId });
    return true;
  }
...
export function endTurn(room: Room): void {
  ...
  const scores = calculateTurnScores(room);
  ...
  if (room.currentDrawerIndex >= room.drawerOrder.length) {
    ...
    // More drawers, next turn
    setTimeout(() => transitionToWordSelection(room), 3000);
  }
}
...
export function endRound(room: Room): void {
  ...
  if (room.roundNumber >= room.totalRounds) {
    ...
    room.roundNumber++;
    room.currentDrawerIndex = 0;
    room.drawerOrder = shuffle(room.drawerOrder); // Reshuffle for next round
