...
import {
  ...
  RoomSocketEvents,
  SocketEvents
} from '../../constants/events/socket.js';
import Controller, { ControllerInterface } from '../../controllers/index.js';
import RoomServiceInstance from '../../services/room/RoomService.js';
import { ClientToServerEvents, IoType, SocketType } from '../../types/socket/index.js';
import { DoodleServerError } from '../../utils/error.js';

import { SocketServiceInterface } from './interface.js';
...
class SocketService implements SocketServiceInterface {
  ...
  public start: SocketServiceInterface['start'] = (io) => {
    ...
    io.on('connection', (socket) => {
      ...
      this._registerK1CompatibilityEvents(socket);
  ...
  /**
   * K1.0 Compatibility Layer
   * Bridges old events to new RoomService/GameService
   */
  private _registerK1CompatibilityEvents(socket: SocketType) {
    // K1.0 Lobby: Get Rooms
    socket.on('lobby:get-rooms', async () => {
      try {
        const roomList = await RoomServiceInstance.listLobbyRoomSummaries();
        socket.emit('lobby:rooms', { rooms: roomList });
      } catch (e) {
        console.error('K1 Compatibility Error (lobby:get-rooms):', e);
      }
    });
    ...
    socket.on('room:create', async (data: any) => {
      try {
        ...
        // Map K1 settings to K2 room creation
        // Note: K2 uses ownerId for private rooms. 
        // We'll use the socket ID as a temporary ownerId if no doodler is set yet.
        ...
        // Also broadcast updated lobby to everyone
        const roomList = await RoomServiceInstance.listLobbyRoomSummaries();
        this._io?.emit('lobby:rooms-updated', { rooms: roomList });
      } catch (e) {
        console.error('K1 Compatibility Error (room:create):', e);
        socket.emit('room:created', { error: 'Failed to create room' });
      }
    });
  }
