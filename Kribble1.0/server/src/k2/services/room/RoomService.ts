...
import { GameStatus, RoomInfoMapType } from '../../types/game.js';
...
import DoodlerServiceInstance from '../doodler/DoodlerService.js';
import GameServiceInstance from '../game/GameService.js';
import { RoomServiceInterface } from './interface.js';
...
class RoomService implements RoomServiceInterface {
  ...
  /**
   * Create a new room
   * @param ownerId - Doodler ID who is the owner of this new room. If undefined, the first to join will become the owner.
   * @param name - Custom room name
   * @param capacity - Room capacity
   * @param isPrivate - Whether the room is private
   * @returns Room Interface of the newly created room
   */
  public async createRoom(ownerId?: string, name?: string, capacity?: number, isPrivate = false) {
    const room = new RoomModel(ownerId, name, capacity, isPrivate);
  ...
  /**
   * Create a new room
   * @param ownerId - Doodler identity
   * @param name - Room name
   * @param capacity - Room capacity
   * @param isPrivate - Whether the room is private
   * @returns Room model of the newly created room
   */
  private async _createRoomModel(ownerId?: string, name?: string, capacity?: number, isPrivate = false) {
    const room = new RoomModel(ownerId, name, capacity, isPrivate);
    this._rooms.set(room.id, room);
    return room;
  }
