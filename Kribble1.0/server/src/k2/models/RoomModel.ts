import { DEFAULT_CAPACITY } from '../constants/game.js';
import { DoodlerInterface } from '../types/socket/doodler.js';
import { generateId } from '../utils/unique.js';
...
export class RoomModel {
  ...
  constructor(ownerId?: string, name?: string, capacity?: number, isPrivate = false) {
    this.id = generateId(); // TODO: handle collision
    this.name = name || `Room ${this.id.slice(0, 4)}`;
  ...
  public addDoodler(doodlerId: string) {
    ...
    if (!this._ownerId) this._ownerId = doodlerId;
    return true;
  }
